...
snakemake -s mixarray_p2.snakefile --latency-wait 60 --keep-going --cluster-config config/cluster_slurm.yaml --cluster "sbatch -p {cluster.partition} --mem={cluster.mem} -t {cluster.time} -c {cluster.ncpus} -n {cluster.ntasks} -o {cluster.output} -J {cluster.JobName}" -j 40 -np
"""
...
rule all:
	input: 
		...
		expand("results/downsamples/{samples}_CODRdownsample.bam", samples=config["samples"])
...
rule downsample:
	...
	output:
		outBam = "results/downsamples/{samples}_CODRdownsample.bam"
	params:
		mem = config["java_mem"],
