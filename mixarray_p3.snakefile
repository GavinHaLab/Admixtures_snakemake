...
def getMergeInput(sampleID):
  return glob.glob(''.join(["results/downsamples/", sampleID, "_CODRdownsample.bam"]))
...
rule all:
	input: 
		...
		expand("results/{mixtures}/{mixtures}_merge.bam", mixtures=config["mixtures"]),
		expand("results/{mixtures}/{mixtures}_merge.bai", mixtures=config["mixtures"]),
		expand("results/{mixtures}/{mixtures}_merge.bam.bai", mixtures=config["mixtures"]),
...
rule mergeFiles:
	...
	output:
		outBam = "results/{mixtures}/{mixtures}_merge.bam",
		outBai = "results/{mixtures}/{mixtures}_merge.bai"
	params:
		mem = config["java_mem"],
...
rule createSymlinks:
	input:
		inBai = "results/{mixtures}/{mixtures}_merge.bai"
	output:
		outBamBai = "results/{mixtures}/{mixtures}_merge.bam.bai"
	params:
		relpath = "{mixtures}_merge.bai"
	log:
...
rule createMergeMetrics:
	input:
		inBam = "results/{mixtures}/{mixtures}_merge.bam"
	output:
