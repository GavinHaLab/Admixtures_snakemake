...
snakemake -s mixarray_p1.snakefile --latency-wait 60 --keep-going --cluster-config config/cluster_slurm.yaml --cluster "sbatch -p {cluster.partition} --mem={cluster.mem} -t {cluster.time} -c {cluster.ncpus} -n {cluster.ntasks} -o {cluster.output} -J {cluster.JobName}" -j 40 -np
"""
...
rule all:
	input: 
		...
		expand("results/sources/{samples}-chrsOnly.bam", samples=config["samples"]),
		expand("results/sources/{samples}-chrsOnly.dups_removed.bam", samples=config["samples"]),
...
rule extractChromosomal:
	...
	output:
		outBam = "results/sources/{samples}-chrsOnly.bam"
	params:
		sam = config["samtools"],
		chrs = config["chrs"]
	log:
...
rule removeDuplicates:
	input:
		inBam = "results/sources/{samples}-chrsOnly.bam"
	output:
		outBam = "results/sources/{samples}-chrsOnly.dups_removed.bam",
...
rule createCODRmetrics:
	input:
		inBam = "results/sources/{samples}-chrsOnly.dups_removed.bam"
	output:
